...
class indicadoresController
{
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            if ($rol == 'Administrador') {
                ...
                return $this->container->get('view')
                    ->render($response, 'home_indicadores.html', [
                        'rol' => $rol,
    ...
    public function verAgregarIndicador(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            return $this->container->get('view')->render($response, 'add_indicador.html', [
                'rol' => $rol,
                'nombre' => $nombre_usuario,
                'dependencia' => $dependencia_usuario,
                'pp' => $pp,
    ...
    public function agregarIndicador(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            if (
                !empty($input['responsable-indicador']) && !empty($input['corresponsable-indicador']) && !empty($input['numeroindicador']) && !empty($input['mes-indicador']) && !empty($input['valor-b']) &&
                !empty($input['valor-c'])
            ) {
                try {
    ...
    public function editarIndicador(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            if (
                !empty($input['responsable-indicador']) && !empty($input['corresponsable-indicador']) && !empty($input['mes-indicador']) && !empty($input['valor-b']) &&
                !empty($input['valor-c'])
            ) {
                try {
    ...
    public function verFicha(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            $file_search = Ficha::where('numero_indicador', '=', $indicador)->get('ficha');
