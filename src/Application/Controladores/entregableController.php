...
class entregableController {
    ...
    public function mostrarEntregable(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        return $this->container->get('view')->render($response, 'home_entregable.html',['nombre'=>$nombre_usuario,'dependencia'=>$dependencia_usuario,
        'rol'=>$rol,'programa'=>$programa,'entregable'=>$entregable,'avance'=>$avance,'idprograma'=>$id,'pespecial'=>$pespecial,'año'=>$año]);
        }
    }
    ...
    public function mostarAgregarEntregable(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        return $this->container->get('view')->render($response, 'add_entregable.html',['nombre'=>$nombre_usuario,'dependencia'=>$dependencia_usuario,'rol'=>$rol,
        'id'=>$id, 'ods'=>$ods, 'avg'=>$avg, 'compromisos'=>$compromisos,
            'pmp'=>$pmp, 'objetivopmp'=>$objetivopmp, 'lineaccionpmp'=>$lineaccionpmp, 'estrategiapmp'=>$estrategiapmp,'rolUser'=>$dataUser[0]->rol_usuario]);
        }
        }
 ...
 if($userRol  == 'Administrador' || $userRol  == 'Enlace SEMUJERES' || $userRol == 'Enlace GEPEA' || $userRol == 'Admin SEMUJERES-GEPEA'){
  if(isset($input['rowfuente'])){
    ...
    $usuario = $_SESSION['user'];
        ...
        $avginput =$input['avg']; 
        return $this->container->get('view')->render($response, 'add_entregable.html',['nombreentregable'=>$nombreentregable,'periodo'=>$periodicidad,'medida'=>$unidadmedida,
        'meta'=>$meta,'municipalizable'=>$municipalizable,'compromiso'=>$compromiso,'odsinput'=>$odsinput,'actsigo'=>$actividad_sigo,'entregablesigo'=>$entregable_sigo,'avginput'=>$avginput,
            'message'=>$msg,'class'=>$class,'nombre'=>$nombre_usuario,'dependencia'=>$dependencia_usuario,'rol'=>$rol,
        'id'=>$id, 'ods'=>$ods, 'avg'=>$avg, 'compromisos'=>$compromisos,
            'pmp'=>$pmp, 'objetivopmp'=>$objetivopmp, 'lineaccionpmp'=>$lineaccionpmp, 'estrategiapmp'=>$estrategiapmp,'rolUser'=>$dataUser[0]->rol_usuario]);
}
        }
    ...
    public function mostrarEditarEntregable(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        $estrategiapmp = EstrategiaPMP::all();
        $compromisos = Compromiso::all();
            return $this->container->get('view')->render($response, 'edit_entregable.html',['nombre'=>$nombre_usuario,'dependencia'=>$dependencia_usuario,'rol'=>$rol,'id_programa'=>$id_programa,'id_entregable'=>$id_entregable, 'entregable'=>$entregable,'compromisos'=>$compromisos,
            'pmp'=>$pmp, 'objetivopmp'=>$objetivopmp, 'lineaccionpmp'=>$lineaccionpmp, 'estrategiapmp'=>$estrategiapmp,'finanzas'=>$json,'pespecial'=>$pEspecial,'rolUser'=>$dataUser[0]->rol_usuario,'ods'=>$ods]);
    }
        }
...
$usuario = $_SESSION['user'];
        ...
        $estrategiapmp = EstrategiaPMP::all();
        $compromisos = Compromiso::all();
            return $this->container->get('view')->render($response, 'edit_entregable.html',['message'=>$msg,'class'=>$class,'nombre'=>$nombre_usuario,'dependencia'=>$dependencia_usuario,'rol'=>$rol,'id_programa'=>$id_programa,'id_entregable'=>$id_entregable, 'entregable'=>$entregable,'compromisos'=>$compromisos,
            'pmp'=>$pmp, 'objetivopmp'=>$objetivopmp, 'lineaccionpmp'=>$lineaccionpmp, 'estrategiapmp'=>$estrategiapmp,'finanzas'=>$json,'pespecial'=>$pEspecial,'rolUser'=>$dataUser[0]->rol_usuario,'ods'=>$ods]);
        }
}
    }
