...
class programasController
{
    ...
    public function mostrarAgregarPrograma(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            $año = date("Y");
            return $this->container->get('view')->render($response, 'add_program.html', [
                'rol' => $rol,
                'nombre' => $nombre_usuario,
                'dependencia' => $dependencia_usuario,
                'eje' => $eje,
                'politica' => $politica,
                'objetivoped' => $objetivoped,
                'estrategiaped' => $estrategiaped,
    ...
    public function agregarPrograma(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            if (
                !empty($input['nombre_programa']) && !empty($input['año']) && !empty($input['objetivo']) && !empty($input['descripcion']) && !empty($input['nombre_responsable']) && !empty($input['cargo_responsable'])
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            if (
                ...
                try {
                    ...
                    if ($programa === null) {
                        $msg = 'El programa que intentas editar no existe.';
                        $class = 'red';
                    } else {
                        $programa->nombre_programa = $input['nombre_programa'];
                        $programa->año = $input['año'];
                        ...
                        if (!empty($input['textareaideje'])) {
                            $totalPed = min(
                                count($input['textareaideje']),
                                count($input['textareapolitica'] ?? []),
                                count($input['textareaobjetivo'] ?? []),
                                count($input['textareaestrategia'] ?? []),
                                count($input['textarealinea'] ?? [])
                            );

                            for ($index = 0; $index < $totalPed; $index++) {
                                $ped = new Alineacionped();
                                $ped->eje = $input['textareaideje'][$index];
                                $ped->politica = $input['textareapolitica'][$index];
                                $ped->objetivo = $input['textareaobjetivo'][$index];
                                $ped->estrategia = $input['textareaestrategia'][$index];
                                $ped->linea = $input['textarealinea'][$index];

                                $programa->alineacionped()->save($ped);
                            }
                        }

                        $msg = 'Tu programa ha sido guardado correctamente.';
                        $class = 'blue';
                    }

                } catch (\PDOException $e) {
                    error_log($e->getMessage());
                    $msg = 'Ocurrio un error al guardar el programa.';
                    $class = 'red';
                }
            } else {
                $msg = 'Tu programa no ha sido guardado porque existen uno o más campos vacios. Llene todos los campos y haga clic en guardar nuevamente.';
